...
// ==========================================
// FIX SENSOR HP: DRAG & DROP BALOK NEON
// ==========================================

function getTouchPos(e) {
    const touch = e.touches[0] || e.changedTouches[0];
    const rect = canvas.getBoundingClientRect();
    // Menghitung posisi jari pas di dalam kotak canvas game
    return {
...
canvas.addEventListener('touchstart', (e) => {
    ...
    if (typeof startDrag === 'function') {
        // Mengirim koordinat posisi jari saat pertama kali menyentuh balok
        startDrag({ clientX: pos.clientX, clientY: pos.clientY });
    }
}, { passive: false });
...
canvas.addEventListener('touchmove', (e) => {
    e.preventDefault();
    const pos = getTouchPos(e);
    // Menyelaraskan fungsi geser mouse laptop agar aktif di jari HP
    if (typeof drag === 'function') {
        drag({ clientX: pos.clientX, clientY: pos.clientY });
    } else if (typeof moveDrag === 'function') {
        moveDrag({ clientX: pos.clientX, clientY: pos.clientY });
    }
}, { passive: false });

canvas.addEventListener('touchend', (e) => {
    e.preventDefault();
    // Membaca fungsi saat jari dilepas dari layar (lepas balok ke grid)
    if (typeof endDrag === 'function') {
        endDrag();
    } else if (typeof stopDrag === 'function') {
        stopDrag();
    }
}, { passive: false });
