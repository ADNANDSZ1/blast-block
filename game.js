...
function getEventPos(e) {
    // Membaca sentuhan jari HP atau klik mouse laptop secara akurat
    const touch = (e.touches && e.touches.length > 0) ? e.touches[0] : 
                  ((e.changedTouches && e.changedTouches.length > 0) ? e.changedTouches[0] : e);
    
    const rect = canvas.getBoundingClientRect();
    
    // RUMUS SAKTI: Menghitung posisi jari berdasarkan skala Canvas yang mengecil di HP
...
function startDrag(e) {
    ...
    if (clickedBlock) {
        ...
        selectedBlock.isDragged = true;
        startX = pos.x - selectedBlock.x;
        startY = pos.y - selectedBlock.y;
    }
}
...
function moveDrag(e) {
    ...
    const pos = getEventPos(e);
    selectedBlock.x = pos.x - startX;
    selectedBlock.y = pos.y - startY;
}
